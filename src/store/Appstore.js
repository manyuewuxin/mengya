...
class Appstore {
    ...
    getPosts(path,page) {
        if(path && page){
            Promise.all([ajax.getPostsList(path, page),ajax.getOrder(),ajax.getUser(),ajax.getMessage(1)]).then(([p,o,u,m])=>{
    ...
    getArticle(posts_id){
        if(posts_id){
            Promise.all([ajax.getArticle(posts_id),ajax.getUser(),ajax.getMessage(1)]).then(([p,u,m])=>{
                this.setState("app",{ 
                    posts:p.posts, 
                    user:u.user,
    ...
    getCollectPosts(collect_id, page){
        if(collect_id && page){
            Promise.all([ajax.getCollect(collect_id, page),ajax.getUser(),ajax.getMessage(1)]).then(([c,u,m])=>{
                this.setState("app",{
                    user: u.user, 
                    posts:c.posts, 
                    collect_user:c.user, 
    ...
    setter_message(open, left) {
        if (this.header.open === open) {
            ...
            ajax.updateMessage().then(()=>this.setState("header", { open: open, left: left, read_count: 0, init: false }));
        } 
        else {
    ...
    @action login(pathname, message = null) {
    ...
    @action like(index, posts_id, author_id) { 
    ...
    @action comment_count(type,index){
        if(type == "add"){
    ...
    @action follow(author_id) {
        if (typeof author_id == "string" && this.id !== author_id) {
            ...
            ajax.following({ author_id, action:actionType }).then(
                action(() => {
                    indexOf !==-1? this.app.user.following.splice(indexOf,1) : this.app.user.following.push(author_id);
                })
            );
        }
        else {
    ...
    @action follow_type(type){
        if(typeof type === "string") {
            ...
            ajax.followtype({ type: type, action: actionType }).then(action(()=>{
                indexOf!==-1 ? this.app.user.followtype.splice(indexOf,1) : this.app.user.followtype.push(type);
            }));
        }
    }

    @action setMessage(message) {  //propmptMessage
    ...
    @action read_article_comment(type,index,top){
        if(type==="article" && typeof index == "number" && Number.isNaN(index) === false && typeof top == "number"){
        ...
        else if(type==="comment" && typeof index == "number" && Number.isNaN(index) === false){
            const indexOf = this.app.read_comment.indexOf(index);
            indexOf !== -1 ? this.app.read_comment.splice(indexOf,1) : this.app.read_comment.push(index);
        }
        else{
