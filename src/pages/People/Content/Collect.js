...
export default class Collect extends Component {
    ...
    render() {
        this.is_myhome = this.props.Appstore.id === this.props.match.params.id ? true : false; //追踪
        ...
        const list = people.map((collect, index) => {
            ...
            return (
                <li
                    ...
                    <div>
                        <p>{collect.name}</p>
                        <p className="information_text">{`${dt.date} 创建 · 收藏 ${collect.collect_count} 篇文章`}</p>
                    </div>
                    <div>
                        <Link to={`/collect/${collect._id}`}>查看</Link>
                        {this.is_myhome ? <a onClick={this.remove} data-index={index}>删除</a> : null}
                    </div>
                </li>
            );
        });
