...
export default class ArticleHtml extends Component {
    ...
    render() {
        ...
        const html = article.image===null ? article.html : `<div><img src=${article.image}></div>${article.html}`;
        const innerHTML = open_article ? html : text;
        const dt = utils.format(article.date);

        return (
            <div>
                <h2 className="posts_title"><Link to={`/p/${article._id}`}>{article.title}</Link></h2>
                <div className="posts_html" dangerouslySetInnerHTML={{ __html: innerHTML }}></div>
                {open_article ? <div className="posts_date">{`发布于 ${dt.date} ${dt.time}`}</div> : null}
            </div>
        );
    }
} 
