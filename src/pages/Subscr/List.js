	...
	render(){
			...
			return(
    			<li key={Math.random().toString(36).substring(2, 6)}>
      				<div className="label_describes">
        				...
        				<button className={isFollow?"is_follow_button":"no_follow_button"} data-index={index} onClick={this.props.follow_type}>
        					{isFollow?"已关注":"关注"}
        				</button>
      				</div>
    			</li>
		...
		return(
			<div className="labelall">
				{label.length===0?<div className="text-center">没有找到该标签</div>:<ul className="labellist">{labelList}</ul>}
  			</div>
		);
	}
}
