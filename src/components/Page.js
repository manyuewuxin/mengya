...
export default class Page extends Component {
    ...
    init(start, count) { //当前页，父组件传递最大页数
    ...
    setList(index, page) { //点击的索引，点击的页数
    ...
    build(page,count){ //开始构建
        const style  = this.props.currentStyle || "";
        this.list = this.arr.map((listpage, index) => { //列表
        ...
        if(page >= 5){
            this.list.unshift(
                <React.Fragment key={Math.random().toString(36).substring(2, 6)}>
                    <li
                    ...
                    <li className="current_page" key={Math.random().toString(36).substring(2, 6)}>...</li>
                </React.Fragment>    
            );
        }

        if(page !== 1){ //上一页按钮
            this.list.unshift(<li onClick={this.back} key={Math.random().toString(36).substring(2, 6)}>上一页</li>);
        }
        ...
        if(page !== count) { //下一页按钮
            this.list.push(<li onClick={this.go} key={Math.random().toString(36).substring(2, 6)}>下一页</li>);
        }
    }
    render() {
