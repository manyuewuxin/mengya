    ...
    render(){ 
    	...
    	return(
            <div className="commect_action">
                <button data-action="good" data-index={index} className={is_good ? "is_good" : ""}>{comment.good.length}</button>

                <button data-action="reply" data-index={index}>回复</button>

                {user_id === author[0]._id ? <button data-action="remove" data-index={index}>删除</button> : null}
            </div>
        );
    }
}