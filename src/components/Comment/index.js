...
export default class Comment extends Component {
    ...
    getComment(posts_id,currentPage,sort){
        ajax.getComment(posts_id, currentPage, sort).then(({ comment, count, page }) => {
            const timeID = window.setTimeout(() => {
                this.setState({ comments:comment, loading: false, currentPage, count, page, sort });
    ...
    good(e) {
     ...
     create(value) {
        if (/^.{1,300}$/.test(value)) {
            ...
            this.props.Appstore.setMessage({ text: "评论内容限制在1-300个字节内", is: false });
        }
    }
    ...
    onRemove(){
        ...
        if(user[0] === this.props.Appstore.id){
            ajax.removeComment({ comment_id, posts_id, author_id, user }).then(() => {
                ...
                this.setState({ comments:comments, count: count-1, visibility: false, description: null });
    ...
    remove(e) {
        ...
        this.setState({ visibility: true, description: { title:"你确定要删除这条评论吗？" } });
    }
    ...
    render() {
        const { sort, comments, loading, count, page, visibility, description } = this.state;
        return (
                ...
                <div className="comment" ref={this.refc}>
                    ...
                    { loading ? <Loading/> : 
                        <React.Fragment key="abcd">
                            <CommentList comments={comments} user_id={this.props.Appstore.id} action={this.action}/> 
                            <CommentPage ulStyle="comment_page" currentStyle="current_page" count={page} setPage={this.setPage} />
                        </React.Fragment>
                    }
                    <CommentTextarea refx={this.refx} create={this.create} />
                </div>
                <CommentRmoeve visibility={visibility} description={description} onRemove={this.onRemove} offRemove={this.offRemove}/>
            </React.Fragment>
        );
    }
    componentDidMount() {
