...
export default class Index extends Component {
    ...
    render(){
        ...
        return(
            <Portal>
                <div className="remove" ref={this.refc} onAnimationIteration={this.animationiteration} onAnimationEnd={this.animationend}>
                    <h3>{this.props.description.title}</h3>
                    <div>
                        <button onClick={this.no}>取消</button>
                        <button onClick={this.ok} className="no_follow_button">确定</button>
                    </div>
                </div>
            </Portal>
        );
