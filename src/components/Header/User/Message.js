...
export default class Message extends Component {
    ...
    action(e) {
        e.stopPropagation();
        this.props.Appstore.setter_message("message", e.target.parentElement.offsetLeft - 165 + 15);
    }
    close(e) {
        e.stopPropagation();
    ...
    render() {
        ...
        return (
            <div className="user_message" onMouseLeave={this.close}>
                ...
                {read_count > 0 ? <div className="count">{read_count < 99 ? read_count : "99+"}</div> : null}
                {open === "message" ? <MessageList left={left} message={message} scroll={this.scroll} /> : null}
            </div>
        );
    }
}
