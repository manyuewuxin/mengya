...
export default class MessageList extends Component {
    ...
    render() {
        const list = this.props.message.map((m) => {
            ...
            if (m.message.message_type === "reply_posts") { //优化合并，验证第三个参数arr本身
                return (
                    <li key={key}>
                        <Link to={`people/${m.author[0]._id}/dynamic`}>{m.author[0].name}</Link>
                        <span>评论了你的文章</span>
        ...
        return (
            <div style={{ position: "fixed", left: `${this.props.left}px`, zIndex: "4" }}>
                <div className="show_message">
                    {list.length === 0 ? <div>还没有任何消息推送</div> : <ul onScroll={this.props.scroll}>{list}</ul>}
                </div>
            </div>
        );
    }
